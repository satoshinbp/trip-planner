...
export default (props) => {
  ...
  const handleEdit = (id) => () => {
    const { title, startDate, endDate, location, note } = trips.filter(
      (trip) => trip.id === id
    )[0]
          ...
          {stableSort(trips, getComparator(order, orderBy))
            .filter((trip) => {
              ...
              switch (filter) {
                ...
                case 'upcoming':
                  return (
                    isEqual(trip.endDate, today) || isAfter(trip.endDate, today)
                  )
                case 'past':
              ...
              <TableRow key={trip.id}>
                ...
                >
                  ...
                  <Tooltip title="Delete">
                    <IconButton
                      onClick={() => setDeleteTrip({ open: true, id: trip.id })}
                    >
                      <DeleteIcon />
                    </IconButton>
                  </Tooltip>
                ...
                <TableCell
                  onClick={linkToTripPage(trip.id)}
      ...
      <Dialog open={deleteTrip.open} onClose={handleClose}>
        ...
        <DialogActions>
          ...
          <Button
            variant="contained"
            color="secondary"
            onClick={handleDelete(deleteTrip.id)}
          >
            Delete
          </Button>
        </DialogActions>
