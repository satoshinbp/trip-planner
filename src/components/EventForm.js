...
export default (props) => {
        ...
        case "edit":
          ...
          eventsRef
            ...
            .then(() => {
              ...
              setEvents(
                [...untouchedEvents, { id: action.id, ...newEvent }].sort((a, b) => {
                  if (isAfter(b.startTime, a.startTime)) return -1;
                  if (isAfter(a.startTime, b.startTime)) return 1;
                  return 0;
                })
  ...
  const renderContent = (category) => {
    switch (category) {
      ...
      case "restaurant":
      case "shopping":
        return <EventFormBasic newEvent={newEvent} setNewEvent={setNewEvent} result={result} dates={dates} />;
      case "hotel":
        return <EventFormHotel newEvent={newEvent} setNewEvent={setNewEvent} result={result} dates={dates} />;
      case "transportation":
        return <EventFormTransport newEvent={newEvent} setNewEvent={setNewEvent} result={result} dates={dates} />;
      default:
        break;
    }
  };
  ...
  useEffect(
    () => setNewEvent(action.mode === "edit" ? events.filter((event) => event.id === action.id)[0] : def.none),
    [action.mode]
  );
