...
export default withAuth((props) => {
  ...
  const TripSummary = () => (
    <Grid container justify="space-between" alignItems="flex-end" className={classes.title}>
      ...
      <Grid item>
        <Hidden xsDown>
          <Button variant="contained" color="primary" className={classes.btn} onClick={() => handleAddEvent(dates[0])}>
            Add a Event
          </Button>
        </Hidden>
        <Hidden smUp>
      ...
      <>
        ...
        <Grid item container className={classes.link} alignItems="center">
          ...
          <IconButton
            edge="end"
            color="secondary"
            disabled={event.location ? !event.location.lat : !event.origin.lat && !event.destination.lat}
            onClick={() => {
  ...
  useEffect(() => {
    ...
    const eventToBeSortedByStartTime = eventsWithOrder.map((event) => ({ ...event, sortTime: event.startTime }));
